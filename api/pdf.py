...
def build_klient_chek(klient_nom, ops_grouped, sana, qarz_tarkib=None):
    ...
    for item in ops_grouped:
        ...
        if avvalgi > 0:
            story.append(row('  Qarz:', f'-{avvalgi:.2f}g', cb=C_RED))
        if tolov_g > 0 and tolov_summa > 0:
            story.append(row(f'  Tolov: {tolov_summa:,.0f}$/{tolov_kurs:.1f}$/g',
                             f'+{tolov_g:.2f}g', cb=C_GREEN))
        elif tolov_g > 0:
            story.append(row('  Tolov:', f'+{tolov_g:.2f}g', cb=C_GREEN))
        if vozvrat_g > 0:
            story.append(row('  Vozvrat:', f'+{vozvrat_g:.2f}g', cb=C_GREEN))
